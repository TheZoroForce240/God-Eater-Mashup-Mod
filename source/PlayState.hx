...
class PlayState extends MusicBeatState
{
	...
	public static var keyAmmo:Array<Int> = [4, 6, 9, 10];
	...
	var tordbot:Character;
	var shaggy:Character;

	var bf_cutscene:Character;

	var isbf:Bool = false;
	var isshaggy:Bool = false;
	var iswhitty:Bool = false;
	var istricky:Bool = false;
	var isopheebop:Bool = false;
	var isbob:Bool = false;
	var isagoti:Bool = false;
	var ispico:Bool = false;
	var istabi:Bool = false;
	var iszardy:Bool = false;
	var ismatt:Bool = false;
	var issarv:Bool = false;
	var isruv:Bool = false;
	var istankman:Bool = false;
	var istordbot:Bool = false;
	...
	var therock2:FlxSprite;
	var therock3:FlxSprite;
	var therock4:FlxSprite;
	var therock5:FlxSprite;
	...
	override public function create()
	{
		...
		else if (SONG.song.toLowerCase() == 'god-eater')
		{
			...
			*/
			...
			therock = new FlxSprite(20, 20);
			therock.frames = Paths.getSparrowAtlas('god_bg');
			therock.animation.addByPrefix('rock', "rock", 30);
			therock.animation.play('rock');
			therock.scrollFactor.set(1, 1);
			therock.antialiasing = true;

			therock2 = new FlxSprite(20, 20);
			therock2.frames = Paths.getSparrowAtlas('god_bg');
			therock2.animation.addByPrefix('rock', "rock", 30);
			therock2.animation.play('rock');
			therock2.scrollFactor.set(1, 1);
			therock2.antialiasing = true;

			therock3 = new FlxSprite(20, 20);
			therock3.frames = Paths.getSparrowAtlas('god_bg');
			therock3.animation.addByPrefix('rock', "rock", 30);
			therock3.animation.play('rock');
			therock3.scrollFactor.set(1, 1);
			therock3.antialiasing = true;
			add(therock3);

			therock4 = new FlxSprite(20, 20);
			therock4.frames = Paths.getSparrowAtlas('god_bg');
			therock4.animation.addByPrefix('rock', "rock", 30);
			therock4.animation.play('rock');
			therock4.scrollFactor.set(1, 1);
			therock4.antialiasing = true;

			therock5 = new FlxSprite(20, 20);
			therock5.frames = Paths.getSparrowAtlas('god_bg');
			therock5.animation.addByPrefix('rock', "rock", 30);
			therock5.animation.play('rock');
			therock5.scrollFactor.set(1, 1);
			therock5.antialiasing = true;
		...
		bf_cutscene = new Character(770, 450, 'reg_bf');
		...
		tabi = new Character(0, 0, 'tabi');
		agoti = new Character(0, 0, 'agoti');
		tordbot = new Character(-800, -16400, 'tordbot');
		tordbot.scale.set(1.5, 1.5);
		zardy = new Character(0, 0, 'zardy');
		bob = new Character(0, 0, 'bob');
		whitty = new Character(0, 0, 'whitty');
		matt = new Character(0, 0, 'matt');
		pico = new Character(0, 0, 'pico');
		ruv = new Character(0, 0, 'ruv');
		opheebop = new Character(0, 0, 'opheebop');
		tankman = new Character(0, 0, 'tankman');
		sarv = new Character(0, 0, 'sarv');
		tricky = new Character(0, 0, 'tricky');
		shaggy = new Character(0, 0, 'pshaggy');
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case "god-eater":
					...
					{
						FlxTween.tween(tordbot, { x: tordbot.x, y: -1400 }, 1);
						godCutEnd = true;
						godMoveGf = true;
						godMoveSh = true;
						...
						new FlxTimer().start(0.002, function(cbt:FlxTimer)
							{
								if (ctrTime == 0)
									{
										var cText = "Q      W     E      R    C/T    N/Y    U      I     O     P";
					
										if (FlxG.save.data.dfjk == 0)
										{
											cText = "Q      W     E      R    C/T    N/Y    U      I     O     P";
										}
										else if (FlxG.save.data.dfjk == 2)
										{
											cText = "Q      W     E      R    C/T    N/Y    U      I     O     P";
										...
										notice = new FlxText(0, 0, 0, cText, 32);
										notice.x = (FlxG.width * 0.5 + 100 + 55) - 590;
										notice.y = 20;
										if (FlxG.save.data.downscroll)
										{
											notice.y = FlxG.height - 200;
				...
				default:
					...
					switch (mania)
					{
						...
						case 2:
							...
							pPre = ['left', 'down', 'up', 'right', 'white', 'yel', 'violet', 'jonquil', 'dark'];
							babyArrow.x -= Note.tooMuch;
						case 3:
							nSuf = ['LEFT', 'DOWN', 'UP', 'RIGHT', 'SPACE', 'SPACE', 'LEFT', 'DOWN', 'UP', 'RIGHT'];
							pPre = ['left', 'down', 'up', 'right', 'white', 'orange', 'yel', 'violet', 'jonquil', 'dark'];
							babyArrow.x -= 350;
					}
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'sky':
				...
				if (godCutEnd)
				{
					...
					var rock_toy = -1600 - Math.sin(rotRate) * 20;

					var sh_toy = -2350 + -Math.sin(rotRateSh * 2) * sh_r * 0.45;
					var sh_tox = -530 -Math.cos(rotRateSh) * sh_r;

					var therock_toy = -1850 - -Math.sin(rotRateSh * 1.4) * sh_r * 1;
					var therock_tox = -430 - -Math.cos(rotRateSh / 2) * sh_r * 1.5;

					var rock4_toy = -3050 - -Math.sin(rotRateSh * 2.1) * sh_r * 0.45;
					var rock4_tox = 330 - -Math.cos(rotRateSh) * sh_r;

					var rock5_toy = -850 + -Math.sin(rotRateSh * 2.2) * sh_r * 0.45;
					var rock5_tox = 1000 - -Math.cos(rotRateSh) * sh_r;
					...
					if (godMoveBf)
					{
						...
						rock.alpha = 1;
						boyfriend.alpha = 1;
						bf_cutscene.visible = false;

						rock.x = boyfriend.x - 100;
						rock.y = boyfriend.y + 200;

						//therock.y = boyfriend.y + 400;
						therock.alpha = 1;

						therock2.x = boyfriend.x - 2300;
						therock2.y = boyfriend.y - 200;
						therock2.alpha = 1;

						therock3.x = boyfriend.x + 1000;
						therock3.y = boyfriend.y + 0;
						therock3.alpha = 1;

						tankman.x = therock2.x + 400;
						tankman.y = therock2.y - 300;
						tankman.alpha = 1;

						zardy.x = therock2.x - 100;
						zardy.y = therock2.y - 550;
						zardy.alpha = 1;
						...
						sarv.x = boyfriend.x + 400;		
						sarv.y = boyfriend.y - 1000;
						sarv.alpha = 1;

						matt.x = therock3.x - 50;		
						matt.y = therock3.y - 300;
						matt.alpha = 1;

						agoti.x = therock3.x + 200;		
						agoti.y = therock3.y - 400;
						agoti.alpha = 1;
					...
					if (godMoveSh)
					{
						...
						remove(dad);
						shaggy.x += (sh_tox - shaggy.x) / 12;
						shaggy.y += (sh_toy - shaggy.y) / 12;

						therock4.x += (rock4_tox - therock4.x) / 12;
						therock4.y += (rock4_toy - therock4.y) / 12;

						therock.x += (therock_tox - therock.x) / 12;
						therock.y += (therock_toy - therock.y) / 12;

						ruv.x = therock.x + 100;		
						ruv.y = therock.y - 700;

						tricky.x = therock.x + 260;		
						tricky.y = therock.y - 540;

						bob.x = therock4.x + 300;		
						bob.y = therock4.y - 300;

						opheebop.x = therock4.x - 10;		
						opheebop.y = therock4.y - 450;

						therock5.x += (rock5_tox - therock5.x) / 12;
						therock5.y += (rock5_toy - therock5.y) / 12;

						whitty.x = therock5.x + 300;		
						whitty.y = therock5.y - 550;

						tabi.x = therock5.x - 10;		
						tabi.y = therock5.y - 550;
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null && !songEnded)
		{
			...
			if (!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
				{
					if (issarv)
						camFollow.setPosition(sarv.getMidpoint().x - 100, sarv.getMidpoint().y - 100);
					else if (isshaggy)
						camFollow.setPosition(shaggy.getMidpoint().x - 100, shaggy.getMidpoint().y - 100);
					else if (isbob)
						camFollow.setPosition(bob.getMidpoint().x - 100, bob.getMidpoint().y - 100);
					else if (isopheebop)
						camFollow.setPosition(opheebop.getMidpoint().x - 100, opheebop.getMidpoint().y - 100);
					else if (isruv)
						camFollow.setPosition(ruv.getMidpoint().x - 100, ruv.getMidpoint().y - 100);
					else if (ismatt)
						camFollow.setPosition(matt.getMidpoint().x - 100, matt.getMidpoint().y - 100);
					else if (ispico)
						camFollow.setPosition(pico.getMidpoint().x - 100, pico.getMidpoint().y - 100);
					else if (istricky)
						camFollow.setPosition(tricky.getMidpoint().x - 100, tricky.getMidpoint().y - 100);
					else if (istankman)
						camFollow.setPosition(tankman.getMidpoint().x - 100, tankman.getMidpoint().y - 100);
					else if (iszardy)
						camFollow.setPosition(zardy.getMidpoint().x - 100, zardy.getMidpoint().y - 100);
					else if (isagoti)
						camFollow.setPosition(matt.getMidpoint().x - 100, matt.getMidpoint().y - 100);
					else if (istabi)
						camFollow.setPosition(tabi.getMidpoint().x - 100, tabi.getMidpoint().y - 100);
					else if (iswhitty)
						camFollow.setPosition(whitty.getMidpoint().x - 100, whitty.getMidpoint().y - 100);
					else if (istordbot)
						camFollow.setPosition(tordbot.getMidpoint().x - 100, tordbot.getMidpoint().y - 1000);
					//else
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						else if (mania == 3)
						{
							...
							{
								dadsDir = ['LEFT', 'DOWN', 'UP', 'RIGHT', 'UP', 'DOWN', 'LEFT', 'DOWN', 'UP', 'RIGHT'];
							} //this stuff for 9 key 
			
		
							if (isbf && player1Character != 'bf' || isbf && SONG.mania == 3) //character swtich stuff
								{
									boyfriend.playAnim('sing' + dadsDir[daNote.noteData], true);
									boyfriend.holdTimer = 0;
								}
							if (iswhitty && player1Character != 'whitty' || iswhitty && SONG.mania == 3)
								{
									whitty.playAnim('sing' + dadsDir[daNote.noteData], true);
									whitty.holdTimer = 0;
								}
							if (ismatt && player1Character != 'matt' || ismatt && SONG.mania == 3)
								{
									matt.playAnim('sing' + dadsDir[daNote.noteData], true);
									matt.holdTimer = 0;
								}
							if (isopheebop && player1Character != 'opheebop' || isopheebop && SONG.mania == 3)
								{
									opheebop.playAnim('sing' + dadsDir[daNote.noteData], true);
									opheebop.holdTimer = 0;
								}
							if (isruv && player1Character != 'ruv' || isruv && SONG.mania == 3)
								{
									ruv.playAnim('sing' + dadsDir[daNote.noteData], true);
									ruv.holdTimer = 0;
									FlxG.camera.shake(0.03, 0.03);						
								}
							if (isshaggy && player1Character != 'shaggy' || isshaggy && SONG.mania == 3)
								{
									shaggy.playAnim('sing' + dadsDir[daNote.noteData], true);
									shaggy.holdTimer = 0;
								}
							if (isbob && player1Character != 'bob' || isbob && SONG.mania == 3)
								{
									bob.playAnim('sing' + dadsDir[daNote.noteData], true);
									bob.holdTimer = 0;
								}
							if (ispico && player1Character != 'pico' || ispico && SONG.mania == 3)
								{
									pico.playAnim('sing' + dadsDir[daNote.noteData], true);
									pico.holdTimer = 0;
								}
							if (isagoti && player1Character != 'agoti' || isagoti && SONG.mania == 3)
								{
									agoti.playAnim('sing' + dadsDir[daNote.noteData], true);
									agoti.holdTimer = 0;
								}
							if (istordbot && player1Character != 'tordbot' || istordbot && SONG.mania == 3)
								{
									tordbot.playAnim('sing' + dadsDir[daNote.noteData], true);
									tordbot.holdTimer = 0;
								}
							if (iszardy && player1Character != 'zardy' || iszardy && SONG.mania == 3)
								{
									zardy.playAnim('sing' + dadsDir[daNote.noteData], true);
									zardy.holdTimer = 0;
								}
							if (issarv && player1Character != 'sarv' || issarv && SONG.mania == 3)
								{
									sarv.playAnim('sing' + dadsDir[daNote.noteData], true);
									sarv.holdTimer = 0;
								}
							if (istabi && player1Character != 'tabi' || istabi && SONG.mania == 3)
								{
									tabi.playAnim('sing' + dadsDir[daNote.noteData], true);
									tabi.holdTimer = 0;
								}
							if (istankman && player1Character != 'tankman' || istankman && SONG.mania == 3)
								{
									tankman.playAnim('sing' + dadsDir[daNote.noteData], true);
									tankman.holdTimer = 0;
								}
							if (istricky && player1Character != 'tricky' || istricky && SONG.mania == 3)
								{
									tricky.playAnim('sing' + dadsDir[daNote.noteData], true);
									tricky.holdTimer = 0;
								}
	...
	public function NearlyEquals(value1:Float, value2:Float, unimportantDifference:Float = 10):Bool
		...
		var nn0Hold:Bool = false;
		var nn1Hold:Bool = false;
		var nn2Hold:Bool = false;
		var nn3Hold:Bool = false;
		var nn4Hold:Bool = false;
		var nn5Hold:Bool = false;
		var nn6Hold:Bool = false;
		var nn7Hold:Bool = false;
		var nn8Hold:Bool = false;
		var nn9Hold:Bool = false;
	...
	private function keyShit():Void
	{
		...
		var nn0 = controls.NN0;
		var nn1 = controls.NN1;
		var nn2 = controls.NN2;
		var nn3 = controls.NN3;
		var nn4 = controls.NN4;
		var nn5 = controls.NN5;
		var nn6 = controls.NN6;
		var nn7 = controls.NN7;
		var nn8 = controls.NN8;
		var nn9 = controls.NN9;

		var nn0P = controls.NN0_P;
		var nn1P = controls.NN1_P;
		var nn2P = controls.NN2_P;
		var nn3P = controls.NN3_P;
		var nn4P = controls.NN4_P;
		var nn5P = controls.NN5_P;
		var nn6P = controls.NN6_P;
		var nn7P = controls.NN7_P;
		var nn8P = controls.NN8_P;
		var nn9P = controls.NN9_P;

		var nn0R = controls.NN0_R;
		var nn1R = controls.NN1_R;
		var nn2R = controls.NN2_R;
		var nn3R = controls.NN3_R;
		var nn4R = controls.NN4_R;
		var nn5R = controls.NN5_R;
		var nn6R = controls.NN6_R;
		var nn7R = controls.NN7_R;
		var nn8R = controls.NN8_R;
		var nn9R = controls.NN9_R;
		...
		else if (mania == 2)
		{
			...
			{
				ankey = (nn0P || nn1P || nn2P || nn3P || nn4P || nn5P || nn6P || nn7P || nn8P || nn9P);
				controlArray = [nn0P, nn1P, nn2P, nn3P, nn4P, nn5P, nn6P, nn7P, nn8P, nn9P];
			}
		if (ankey && !boyfriend.stunned && generatedMusic)
			...
			else if (mania == 2)
			{
				...
				{
					condition = ((nn0 || nn1 || nn2 || nn3 || nn4 || nn5 || nn6 || nn7 || nn8 || nn9) && generatedMusic || (nn0Hold || nn1Hold || nn2Hold || nn3Hold || nn4Hold || nn5Hold || nn6Hold || nn7Hold || nn8Hold || nn9Hold) && loadRep && generatedMusic);
				}
			if (condition)
			{
				notes.forEachAlive(function(daNote:Note)
				{
					if (daNote.canBeHit && daNote.mustPress && daNote.isSustainNote)
					{
						...
						else if (mania == 3)
						{
							...
							{
								// NOTES YOU ARE HOLDING
								case 0: if (nn0 || nn0Hold) goodNoteHit(daNote);
								case 1: if (nn1 || nn1Hold) goodNoteHit(daNote);
								case 2: if (nn2 || nn2Hold) goodNoteHit(daNote);
								case 3: if (nn3 || nn3Hold) goodNoteHit(daNote);
								case 4: if (nn4 || nn4Hold) goodNoteHit(daNote);
								case 5: if (nn5 || nn5Hold) goodNoteHit(daNote);
								case 6: if (nn6 || nn6Hold) goodNoteHit(daNote);
								case 7: if (nn7 || nn7Hold) goodNoteHit(daNote);
								case 8: if (nn8 || nn8Hold) goodNoteHit(daNote);
								case 9: if (nn9 || nn9Hold) goodNoteHit(daNote);
							}
						}
					}
				});
			...
			playerStrums.forEach(function(spr:FlxSprite)
			{
				...
				else if (mania == 2)
				{
				...
				else if (mania == 3)
					{
						...
						{
							case 0:
								if (nn0P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn0R) spr.animation.play('static');
							case 1:
								if (nn1P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn1R) spr.animation.play('static');
							case 2:
								if (nn2P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn2R) spr.animation.play('static');
							case 3:
								if (nn3P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn3R) spr.animation.play('static');
							case 4:
								if (nn4P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn4R) spr.animation.play('static');
							case 5:
								if (nn5P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn5R) spr.animation.play('static');
							case 6:
								if (nn6P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn6R) spr.animation.play('static');
							case 7:
								if (nn7P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn7R) spr.animation.play('static');
							case 8:
								if (nn8P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn8R) spr.animation.play('static');
							case 9:
								if (nn9P && spr.animation.curAnim.name != 'confirm') spr.animation.play('pressed');
								if (nn9R) spr.animation.play('static');
						}
					}
		...
		function goodNoteHit(note:Note):Void
			{
				...
				if (!note.wasGoodHit)
				{
					...
					{
						sDir = ['LEFT', 'DOWN', 'UP', 'RIGHT', 'UP', 'DOWN', 'LEFT', 'DOWN', 'UP', 'RIGHT'];
					}
					...
					if (isopheebop && player1Character == 'opheebop' || isopheebop && SONG.mania == 3)
						{
							opheebop.playAnim('sing' + sDir[note.noteData], true);
							opheebop.holdTimer = 0;
						}
					if (isruv && player1Character == 'ruv' || isruv && SONG.mania == 3)
						{
					...
					if (isbob && player1Character == 'bob' || isbob && SONG.mania == 3)
						{
							bob.playAnim('sing' + sDir[note.noteData], true);
							bob.holdTimer = 0;
						}
					if (ispico && player1Character == 'pico' || ispico && SONG.mania == 3)
						{
					...
					if (isagoti && player1Character == 'agoti' || isagoti && SONG.mania == 3)
						{
							agoti.playAnim('sing' + sDir[note.noteData], true);
							agoti.holdTimer = 0;
						}
					if (istordbot && player1Character == 'tordbot' || istordbot && SONG.mania == 3)
						{
							tordbot.playAnim('sing' + sDir[note.noteData], true);
							tordbot.holdTimer = 0;
						}
					if (iszardy && player1Character == 'zardy' || iszardy && SONG.mania == 3)
						{
							zardy.playAnim('sing' + sDir[note.noteData], true);
							zardy.holdTimer = 0;
						}
					if (issarv && player1Character == 'sarv' || issarv && SONG.mania == 3)
						{
							sarv.playAnim('sing' + sDir[note.noteData], true);
							sarv.holdTimer = 0;
						}
					if (istabi && player1Character == 'tabi' || istabi && SONG.mania == 3)
						{
							tabi.playAnim('sing' + sDir[note.noteData], true);
							tabi.holdTimer = 0;
						}
					if (istankman && player1Character == 'tankman' || istankman && SONG.mania == 3)
						{
							tankman.playAnim('sing' + sDir[note.noteData], true);
							tankman.holdTimer = 0;
						}
					if (istricky && player1Character == 'tricky' || istricky && SONG.mania == 3)
						{
							tricky.playAnim('sing' + sDir[note.noteData], true);
							tricky.holdTimer = 0;
						}
	...
	override function stepHit() 
	{
		...
		if (curStage == 'sky' && curStep != stepOfLast) //hardcoded song events for character switching
			{
				switch(curStep)
				{
					case 2: 
					...
					case 63: 
						...
						player1Character = 'pico';
					case 127: 
						resetCharacters();
						switchCharacter('shaggy');
						switchCharacter('sarv');
						player1Character = 'sarv';
					case 191: 
						resetCharacters();
						switchCharacter('bf');
					...
					case 255: 
						...
						switchCharacter('tordbot');
						player1Character = 'whitty';
					case 319: 
						resetCharacters();
						switchCharacter('tankman');
					case 351: 
						switchCharacter('tabi');
						player1Character = 'tabi';
					case 415: 
						//where sky would start singing, but she ded
						istankman = false;
					case 479: 
						resetCharacters();
						switchCharacter('pico');
					...
					case 607: 
						isbf = false;
						switchCharacter('opheebop');
						player1Character = 'opheebop';
					case 671: 
						resetCharacters();
						switchCharacter('agoti');
						switchCharacter('sarv');
						player1Character = 'agoti';
					case 735: 
						resetCharacters();
						switchCharacter('ruv');
						switchCharacter('matt');
						player1Character = 'ruv';
					case 799: 
						resetCharacters();
						switchCharacter('bf');
						switchCharacter('agoti');
					...
					case 1503: 
						resetCharacters();
						switchCharacter('shaggy');
						player1Character = 'shaggy';
					case 1567: 
						resetCharacters();
						//where sky appears
						camFollow.setPosition(pico.getMidpoint().x - 100, pico.getMidpoint().y - 1400);
					case 1599: 
						switchCharacter('zardy');
						player1Character = 'zardy';
					case 1727: 
						resetCharacters();
						switchCharacter('tordbot');
					case 1794: 
		...
		new FlxTimer().start(0.002, function(tmr:FlxTimer)
		{
			switch (cs_time)
			{
				case 200:
					...
					cs_cam.y -= 200;
				case 400:
					shaggy.playAnim('smile');
				case 500:
					if (!cs_wait)
					{
						if (fc)
							csDial('sh_fc_amazing');
						else if (storyDifficulty == 0 && theFunne)
							csDial('sh_easyvirgin_amazing');
						else if (storyDifficulty == 0 && !theFunne)
							csDial('sh_easychad_amazing');
						else if (storyDifficulty == 1 && theFunne)
							csDial('sh_hardvirgin_amazing');
						else if (storyDifficulty == 1 && !theFunne)
							csDial('sh_hardchad_amazing');
						else if (storyDifficulty == 2 && theFunne)
							csDial('sh_evenhardervirgin_amazing');
						else if (storyDifficulty == 2 && !theFunne)
							csDial('sh_evenharderchad_amazing');
						else
							csDial('sh_amazing');
						schoolIntro(0);
						cs_wait = true;
				...
				case 700:
					...
					remove(tordbot);
					add(dad);
					if (fc || storyDifficulty == 2 && !theFunne)
					{
						bf_cutscene.visible = true;
				...
				case 800:
					if (!cs_wait)
					{
						if (fc)
						{
							...
							csDial('sh_easyvirgin_expo');
						else if (storyDifficulty == 0 && !theFunne)
							csDial('sh_easychad_expo');
						else if (storyDifficulty == 1 && theFunne)
							csDial('sh_hardvirgin_expo');
						else if (storyDifficulty == 1 && !theFunne)
							csDial('sh_hardchad_expo');
						else if (storyDifficulty == 2 && theFunne)
							csDial('sh_evenhardervirgin_expo');
						else if (storyDifficulty == 2 && !theFunne)
						{
							csDial('sh_evenharderchad_expo');
							goodending = true;
						}
						else
							csDial('sh_expo');
				...
				case 840:
					if (storyDifficulty == 0 && theFunne && !fc)
					{
						cs_wait = true;
						badending = true;
						health -= 100;
	...
	public function modCredits()
	{
		...
		new FlxTimer().start(3, function(tmrt:FlxTimer)
		{
			...
			new FlxTimer().start(2.5, function(tmrth:FlxTimer)
			{
				...
				new FlxTimer().start(2.5, function(tmrth:FlxTimer)
				{
					endtxt = new Alphabet(6, FlxG.height / 2 + 380, "TRUE END", true, false);
	...
	public function normalmodCredits()
		{
			FlxG.sound.play(Paths.sound('cs_credits'));
			new FlxTimer().start(0.002, function(btmr:FlxTimer)
			{
			...
			new FlxTimer().start(3, function(tmrt:FlxTimer)
			{
				...
				new FlxTimer().start(2.5, function(tmrth:FlxTimer)
				{
					...
					new FlxTimer().start(2.5, function(tmrth:FlxTimer)
					{
						endtxt = new Alphabet(6, FlxG.height / 2 + 380, "NORMAL ENDING", true, false);
						endtxt.screenCenter(X);
						endtxt.x -= 150;
						add(endtxt);
	...
	var sh_kill_line:String = "Maybe if you proved yourself,\nI'd return you to normal.";
	var sh_acc_line:String = "........................................................\nHeh, maybe not.";
	...
	public function csDial(csIndex:String)
	{
		if (accuracy <= 80 && !fc)
			...
			{
				sh_acc_line = ".............................................................\nprobably just a bot. take that as\na compliment if you aren't.";//you will probably never see this in game
			}
		else if (fc)
			{
				sh_acc_line = "...........................................................\nGuess you're just that good then.";
			}
		switch (csIndex)
		{
			case 'found_scooby':
			...
			case "troleo":
				...
				// new diagloue for differnt endings /// cant spell lol//////////////////////////////// (this is pretty cringe so you better get away now before you are blinded by the cringe)
			...
			case "sh_evenhardervirgin_amazing":
				dialogue = [
					"...",
					"Amazing!"
				];
			...
			case "sh_evenharderchad_amazing":
				dialogue = [
					"...",
					"Wow!",
					"You're like, really good man."
				];
				dface = [
						"f_sh_smug",
						"f_sh_smug",
			...
			case 'sh_easychad_expo':
				dialogue = [
					"...",
					"Really",
					"Easy mode.",
					"...",
					"I'l let you live since you're a chad",
			...
			case 'sh_hardvirgin_expo':
				dialogue = [
					"That was a fun rematch man.",
					"I used even more of my power than\nlast time.",
					"Even after summoning your past foes,\nyou still survived.",
					"...",
					"But you probably spammed your way\nthrough that didn't you?",
					"...",
					sh_acc_line
				];
				dface = [
			...
			case 'sh_hardchad_expo':
				dialogue = [
					"That was a fun rematch man.",
					"I used even more of my power than\nlast time.",
					"Even after summoning your past foes,\nyou still survived.",
					"...",
					"At least you were a chad and\n didn't cheat.",
					"..."
				];
				dface = [
						"f_sh_smug",
			...
			case 'sh_fc_expo':
				dialogue = [
					"Wow, that was fun.",
					"I'm suprised you didn't miss at all.",
					"...",
					"You must of practiced a lot.",
					"...",
					sh_acc_line
				];
				dface = [
			...
			case "sh_normal_bye":
				dialogue = [
					...
					"Also by the way, you're like\nan alternate version of yourself now.",
					"BEP?!",
					sh_kill_line
				];
				dface = [
			...
			case "sh_good_bye":
				dialogue = [
					"Anyways, we gotta go now.",
					"Also by the way, I made you normal again\nsince I enjoyed your performance so much.",
					"beep skeboop (epic)"
				];
				dface = [
						"f_sh",
