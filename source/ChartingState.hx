...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var keyAmmo:Array<Int> = [4, 6, 9, 10];
	...
	function addSongUI():Void
	{
		...
		var m_check3 = new FlxUICheckBox(110, 45, null, null, "10", 100);
	...
	override function update(elapsed:Float)
	{
		...
		if (_song.mania == 3 && gridBG.width != S_GRID_SIZE * 20)
			{
				remove(gridBG);
		...
		if (_song.mania == 2 || _song.mania == 3) strumLine.x = -70;
						...
						FlxG.overlap(strumLine, note, function(_, _)
						{
							if(!claps.contains(note))
							{
								if (note.noteData < 4 && _song.notes[curSection].mustHitSection)
									{
										claps.push(note);
										FlxG.sound.play(Paths.sound('difSNAP'));
									}
								if (note.noteData > 3 && !_song.notes[curSection].mustHitSection)
									{
										claps.push(note);
										FlxG.sound.play(Paths.sound('difSNAP'));
									}
